...
function getLatestResultData(found, asset){
  const history = getAssetHistory(asset);
  const latest = history[0];
  const result = String((latest && latest.result) || found.item.result || found.item.visual || '').toUpperCase();
  const good = result === 'PASS';
  return {
    result: result || 'UNKNOWN',
    color: good ? '#14894c' : '#c62828',
    bg: good ? '#e9f7ef' : '#fdecec'
  };
}
...
function ensureScanResultModal(){
    ...
    <div class="modal" style="width:min(94vw,720px)">
      ...
      <div class="modalBody">
        ...
        <div style="display:grid;gap:12px;margin-top:16px">
          ...
          <button type="button" id="scanActionRetest" class="modalBtn" style="background:#c26a00;color:#fff;min-height:52px;font-weight:800">Test This Appliance Again</button>
          <button type="button" id="scanActionNext" class="modalBtn" style="background:#6a4fb3;color:#fff;min-height:52px;font-weight:800">Scan Next</button>
        </div>
      </div>
  ...
  document.getElementById('scanActionRetest').onclick = ()=>testScannedApplianceAgain();
  document.getElementById('scanActionNext').onclick = ()=>scanNextItem();
...
function openScanResultModal(found, asset){
  ...
  const latestResult = getLatestResultData(found, asset);
  ...
  info.innerHTML = `
    ...
    <div style="margin-top:12px;padding:12px 14px;border-radius:12px;background:${latestResult.bg};border:1px solid ${latestResult.color};font-weight:900;color:${latestResult.color};text-align:center">
      Latest Result: ${esc(latestResult.result)}
    </div>
  `;
...
function testScannedApplianceAgain(){
  if(!lastScanFound) return;
  const found = lastScanFound.found;
  closeScanResultModal();
  window.pendingScannedRetest = {
    asset: lastScanFound.asset,
    appliance: found.item.appliance || '',
    location: found.item.location || 'Office',
    classType: found.item.classType || 'Class I',
    fuse: found.item.fuse || '1A',
    earth: found.item.earth || '',
    ir: found.item.ir || '>200 MΩ'
  };
  if($('jobClientPick')) $('jobClientPick').value = '';
  if($('jobDate')) $('jobDate').value = today();
  if($('jobClient')) $('jobClient').value = found.job.client || '';
  ...
  if($('jobEngineer')) $('jobEngineer').value = found.job.engineer || '';
  if($('jobAddress')) $('jobAddress').value = found.job.address || '';
  if($('jobRetest')) $('jobRetest').value = found.job.retest || '12';
  if($('jobNotes')) $('jobNotes').value = found.job.notes || '';
  switchView('jobsView');
  brandedAlert('Site details loaded. Create the new job and the appliance form will be prefilled automatically.');
}
