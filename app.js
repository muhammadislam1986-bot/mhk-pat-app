...
function showAppModal(msg, opts={}){
  ...
  const okBtn = $('mhkModalOk');
  const cancelBtn = $('mhkModalCancel');
  okBtn.textContent = opts.okText || 'OK';
  cancelBtn.textContent = opts.cancelText || 'Cancel';
  okBtn.className = 'modalBtn ' + (opts.okClass || '');
  cancelBtn.className = 'modalBtn ' + (opts.cancelClass || 'ghost');
  okBtn.style.minWidth = '144px';
  okBtn.style.minHeight = '52px';
  okBtn.style.fontWeight = '900';
  cancelBtn.style.minWidth = '144px';
  cancelBtn.style.minHeight = '52px';
...
function brandedAlert(msg){
  return showAppModal(msg, {title:'Notice', confirm:false, okText:'OK', okClass:'green'});
}
...
async function exportBackupNow(){
  ...
  a.download = 'PAT_Backup_' + stamp + '.json';
  document.body.appendChild(a);
  a.click();
  setTimeout(()=>{ URL.revokeObjectURL(a.href); a.remove(); }, 400);

  showBackupInline('info', 'Backup download started. Check your Downloads folder before confirming.');

  const saved = await askConfirm(
    'Did the backup file save successfully? Check your Downloads folder before continuing.',
    {
      title:'Confirm Backup',
      okText:'YES - Saved',
      cancelText:'NO - Not Saved',
      okClass:'green',
      cancelClass:'red'
    }
  );
  if(saved){
    ...
    showBackupInline('success', 'Backup confirmed as saved. Status updated.');
  }else{
    showBackupInline('warning', 'Backup not confirmed. Status was not updated.');
  }
}
...
function askConfirm(msg, opts={}){
  return showAppModal(msg, {
    title: opts.title || 'Confirmation',
    confirm: true,
    okText: opts.okText || 'Yes',
    cancelText: opts.cancelText || 'No',
    okClass: opts.okClass || 'green',
    cancelClass: opts.cancelClass || 'red'
  });
}
...
window.delJob = async id => {
  ...
  const label = job ? `${job.client} at ${job.address}` : 'this job';
  if(!(await askConfirm(
    `Delete job for ${label}? This cannot be undone.`,
    {title:'Delete Job', okText:'Delete Job', cancelText:'Keep Job', okClass:'red', cancelClass:'secondary'}
  ))) return;
  state.jobs = state.jobs.filter(j=>j.id!==id);
  if(state.activeJobId===id) state.activeJobId=null;
  save(); renderJobs(); renderItems();
};
window.delItem = id => {
...
function renderReport(){
  ...
  $('reportArea').innerHTML = ''
    ...
    +         '<div><b>Report Number:</b> ' + esc(reportNo) + '</div>'
    +         '<div><b>Test Date:</b> ' + fmtDateDMY(job.date) + '</div>'
    +         '<div><b>Retest Date:</b> ' + nextTest(job) + '</div>'
    +       '</div>'

    +       '<div class="reportSummaryCard compact">'
    +         '<div class="reportStatGrid full">'
    ...
    +       '<div class="reportBoxSq premium keepTogether compactThanks">'
    +         '<div class="reportSectionTitle">Thank You</div>'
    +         '<div style="line-height:1.55;color:#111">Thank you for choosing <b>' + esc(state.company.name) + '</b>. If you require further electrical safety testing, PAT maintenance, or compliance inspections, please contact us using the details on page 1 of this report.</div>'
    +       '</div>'

    +       '<div class="reportFoot premium">'
    +         '<div>' + esc(state.company.name) + ' · Portable Appliance Testing (PAT)</div>'
    +         '<div>Evidence & Sign-Off</div>'
    +       '</div>'
    +     '</div>'
    +   '</div>'
    + '</div>';
