...
async function exportBackupNow(){
  ...
  const saved = await askConfirm('Did the backup file save successfully? Check your Downloads folder before continuing.');
  if(saved){
    const meta = getBackupMeta();
...
window.delJob = async id => {
  ...
  if(!(await askConfirm(`Delete job for ${label}? This cannot be undone.`))) return;
