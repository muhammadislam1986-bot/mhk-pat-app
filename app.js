...
const defaults = {
  company:{name:'MHK Building Solutions Ltd',service:'Portable Appliance Testing (PAT)',phone:'07876786286',email:'mhkbuildingsolutions@gmail.com',address:'60 Little Horton Lane, Bradford BD5 0BS',logo:'',signature:'',pin:'',testerModel:'Seaward Apollo 400+ PAT Tester',testerSerial:'',testerCalDue:''},
...
function renderReport(){
  ...
  const failWithRef = fail.map((i, idx) => ({...i, photoRef: i.photo ? ('FP-' + (idx+1)) : ''}));
  const failPhotos = failWithRef.filter(i=>i.photo);

  const reportNo = ensureReportNumber(job);
  const logoHtml = state.company.logo ? '<img src="' + state.company.logo + '" alt="Logo" style="max-width:150px;max-height:68px;display:block">' : '';
  const sigHtml = state.company.signature ? '<img src="' + state.company.signature + '" alt="Signature" style="max-width:210px;max-height:26px;display:block">' : '';

  const testerModel = state.company.testerModel || 'Seaward Apollo 400+ PAT Tester';
  const testerSerial = state.company.testerSerial || '____________';
  const testerCalDue = state.company.testerCalDue ? fmtDateDMY(state.company.testerCalDue) : '____________';
  ...
  const failRows = failWithRef.length ? failWithRef.map(i =>
    '<tr>' +
      '<td>' + esc(i.asset) + '</td>' +
      '<td>' + esc(i.appliance) + '</td>' +
      '<td>' + esc(i.location) + '</td>' +
      ...
      '<td>' + esc(i.notes || '') + (i.photoRef ? '<div style="margin-top:4px;font-size:.82rem;color:#4f5e70"><b>Photo Ref:</b> ' + esc(i.photoRef) + '</div>' : '') + '</td>' +
    '</tr>'
  ).join('') : '<tr><td colspan="10">None</td></tr>';

  const photoBlocks = failPhotos.map(i =>
    '<div class="reportPhotoCard">' +
      '<div style="font-weight:800;color:#111">' + esc(i.photoRef) + ' · ' + esc(i.asset) + ' · ' + esc(i.appliance) + '</div>' +
      '<div class="reportMuted" style="margin-top:3px;font-size:.84rem"><b>Failure reason:</b> ' + esc(i.notes || 'See failed items table') + '</div>' +
      '<div style="margin-top:6px"><img src="' + i.photo + '" alt="Failure photo"></div>' +
    '</div>'
  ).join('');
  ...
  $('reportArea').innerHTML = ''
    ...
    +         '<div style="flex:1">'
    +           '<div style="font-size:1.42rem;font-weight:900;color:#111">' + esc(state.company.name) + '</div>'
    +           '<div style="margin-top:4px;font-size:1rem;color:#111">' + esc(state.company.service) + '</div>'
    +           '<div style="margin-top:7px;line-height:1.38;color:#111">' + nl2br(state.company.address) + '<br>' + esc(state.company.phone) + ' · ' + esc(state.company.email) + '</div>'
    +         '</div>'
    +         '<div style="flex:0 0 auto;text-align:right">' + logoHtml + '</div>'
    +       '</div>'
    +       '<div style="margin-top:12px;text-align:center">'
    +         '<div style="font-size:1.14rem;font-weight:900;color:#111;letter-spacing:.22px">PORTABLE APPLIANCE TESTING REPORT</div>'
    +         '<div style="margin-top:5px;color:#111"><b>Report Number:</b> ' + esc(reportNo) + '</div>'
    +         '<div style="margin-top:3px;color:#111"><b>Test Date:</b> ' + fmtDateDMY(job.date) + ' &nbsp; <b>Retest Date:</b> ' + nextTest(job) + '</div>'
    +       '</div>'
    +       '<div class="reportSplit" style="margin-top:10px">'
    ...
    +           '<div style="margin-top:5px"><b>Site Address:</b><br>' + nl2br(job.address) + '</div>'
    +         '</div>'
    +         '<div class="reportBoxSq">'
    +           '<div class="reportSectionTitle">Test Details</div>'
    +           '<div><b>Engineer:</b> ' + esc(job.engineer) + '</div>'
    +           '<div><b>Instrument:</b> ' + esc(testerModel) + '</div>'
    +           '<div><b>Serial Number:</b> ' + esc(testerSerial) + '</div>'
    +           '<div><b>Calibration Due:</b> ' + esc(testerCalDue) + '</div>'
    +         '</div>'
    +       '</div>'
    ...
    +         '<tr><td>' + job.items.length + '</td><td style="color:#14894c;font-weight:800">' + pass.length + '</td><td style="color:#c62828;font-weight:800">' + fail.length + '</td><td>' + nextTest(job) + '</td></tr>'
    +       '</table>'
    +       '<div class="reportBoxSq">'
    ...
    +         '</table>'
    +       '</div>'
    +       '<div class="reportFoot"><div>' + esc(state.company.name) + ' · Report ' + esc(reportNo) + '</div><div>Page 1 of 2</div></div>'
    +     '</div>'
    +   '</div>'
    +   '<div class="reportPage">'
    +     '<div class="reportPageInner">'
    ...
    +         '<div class="reportSectionTitle">Failed Appliance Safety Notice</div>'
    +         '<div style="line-height:1.36;color:#111">Appliances that have failed testing must be removed from service immediately and must not be used until they have been repaired and successfully re-tested. These items may present an electrical safety risk and should be clearly identified and isolated from normal use.</div>'
    +       '</div>'
    +       '<div class="reportBoxSq keepTogether">'
    +         '<div class="reportSectionTitle">Compliance Statement</div>'
    +         '<div style="line-height:1.36;color:#111">Inspection and testing has been carried out in accordance with the Electricity at Work Regulations 1989 and the IET Code of Practice for In-Service Inspection and Testing of Electrical Equipment.</div>'
    +       '</div>'
    +       '<div class="reportBoxSq">'
    +         '<div class="reportSectionTitle">Failure Photos</div>'
    +         (photoBlocks ? '<div class="reportPhotoGrid">' + photoBlocks + '</div>' : '<div style="color:#111">No failure photos attached.</div>')
    +       '</div>'
    +       '<div class="reportBoxSq keepTogether">'
    +         '<div class="reportSectionTitle">Engineer Sign-off</div>'
    +         '<div class="reportSignatureBox">' + sigHtml + '</div>'
    +         '<div style="display:flex;justify-content:space-between;gap:12px;margin-top:7px;color:#111;font-size:.9rem">'
    +           '<div><b>Engineer:</b> ' + esc(job.engineer) + '</div>'
    ...
    +         '<div style="line-height:1.36;color:#111">Thank you for choosing <b>' + esc(state.company.name) + '</b>. If you require further electrical safety testing, PAT maintenance, or compliance inspections, please contact us using the details on page 1 of this report.</div>'
    +       '</div>'
    +       '<div class="reportFoot"><div>' + esc(state.company.name) + ' · Report ' + esc(reportNo) + '</div><div>Page 2 of 2</div></div>'
    +     '</div>'
    +   '</div>'
    + '</div>';
}
...
function loadSettings(){
  ...
  $('setTesterModel').value = state.company.testerModel || '';
  $('setTesterSerial').value = state.company.testerSerial || '';
  $('setTesterCalDue').value = state.company.testerCalDue || '';
...
$('saveSettingsBtn').onclick = ()=>{
  ...
  state.company.testerModel=$('setTesterModel').value.trim();
  state.company.testerSerial=$('setTesterSerial').value.trim();
  state.company.testerCalDue=$('setTesterCalDue').value.trim();
  save(); refreshHeader(); brandedAlert('Settings saved');
};
$('clearDataBtn').onclick = async ()=>{
  if(!(await askConfirm('Reset all app data?'))) return;
...
$('exportDataBtn').onclick = ()=>{
  ...
  a.download = 'mhk-pat-backup-' + stamp + '.json';
...
$('mhkModalOk').onclick = ()=>{
  $('mhkModalBack').style.display='none';
  if(appModalResolver){ const r = appModalResolver; appModalResolver = null; r(true); }
};
$('mhkModalCancel').onclick = ()=>{
  $('mhkModalBack').style.display='none';
  if(appModalResolver){ const r = appModalResolver; appModalResolver = null; r(false); }
};
$('mhkModalBack').onclick = (e)=>{
  if(e.target.id==='mhkModalBack'){
    $('mhkModalBack').style.display='none';
    if(appModalResolver){ const r = appModalResolver; appModalResolver = null; r(false); }
  }
};
