...
function drawThermalLabel(ctx, canvas, data, qrImg=null){
  ...
  const qrSize = compact ? 128 : 145;
  const metaFont = compact ? 21 : 27;
  const applianceFont = compact ? 26 : 31;
  const assetFont = compact ? 44 : 52;
  const resultFont = compact ? 52 : 62;
  const failFont = compact ? 24 : 30;
  ...
  ctx.fillText(String(data.asset || ''), leftX, pad + 54);

  // Appliance (wrap to 2 lines max)
  ctx.font = `${applianceFont}px Arial`;
  wrapText(ctx, String(data.appliance || ''), leftX, pad + 96, maxTextWidth, compact ? 28 : 34, 2);

  // Result
  ctx.font = `bold ${resultFont}px Arial`;
  ctx.fillText(resultLine, leftX, compact ? 210 : 245);

  let metaTop = compact ? 210 : 245;

  if(data.isFail){
    ctx.font = `bold ${failFont}px Arial`;
    ctx.fillText('DO NOT USE', leftX, compact ? 245 : 282);
    metaTop = compact ? 245 : 282;
  }

  ctx.font = `${metaFont}px Arial`;
  ctx.fillText(`T: ${String(data.tested || '')}`, leftX, metaTop + (compact ? 34 : 46));
  ctx.fillText(`R: ${String(data.retest || '')}`, leftX, metaTop + (compact ? 66 : 82));
  if(initials){
    ctx.fillText(`By: ${initials}`, leftX, metaTop + (compact ? 98 : 118));
  }
